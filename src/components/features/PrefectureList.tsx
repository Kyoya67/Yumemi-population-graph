  ...
  const handleCheckboxChange = async (prefecture: Prefecture, checked: boolean) => {
    try {
      ...
      if (checked) {
        ...
        setSelectedPrefectures((prev) => prev.filter((p) => p.prefName !== prefecture.prefName));
      }
    } catch (error) {
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '30px' }}>
        ...
        <select
          ...
          onChange={(e) => handlePopulationTypeChange(e.target.value as PopulationType)}
      ...
      {selectedPrefectures.length > 0 && (
        <PopultionGraph populationData={selectedPrefectures} populationType={populationType} />
      )}
    </div>
  );
};
